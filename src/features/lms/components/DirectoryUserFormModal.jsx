...
export default function DirectoryUserFormModal({ user, role, documentTypes = [], onSave, onClose }) {
  ...
  function handleSubmit(e) {
    ...
    if (isTeacher) {
      ...
      const payload = { name: form.name, email: form.email, phone: form.phone.trim() }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
      ...
      <div className="rounded-2xl p-6 w-full shadow-2xl max-w-md"
        ...
        <p className="text-xs mb-5" style={{ color: 'var(--muted-foreground)' }}>
          El usuario queda guardado en este navegador y ya puede iniciar sesión con el correo y la contraseña indicados.
        </p>
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          ...
          <FormField label="Número de teléfono" required={isTeacher} error={errors.phone}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Contraseña" required={!user} error={errors.password}
              helperText={user ? 'Deje en blanco para no cambiarla.' : 'Mínimo 6 caracteres.'}>
